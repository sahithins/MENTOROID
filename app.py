...
app.config["MONGODB_SETTINGS"] = {
    'db': 'Mentoroid',
    'host': "mongodb+srv://user123:Mentoroid1@cluster0.7kxexwk.mongodb.net/Mentoroid?retryWrites=true&w=majority&appName=mentoroid"
}
        ...
        for user_id in user_ids:
            ...
            if user:
                ...
                enrolled_users_details.append({
                    ...
                    'Email': user.email,
                    'Enrolled Courses': ', '.join(enrolled_course_names_for_user) 
                })
...
def user_materials():
    ...
    if selected_course_id:
        ...
        if user and selected_course_obj and selected_course_obj.course_name in enrolled_courses_names:
            ...
            quiz = Quiz.objects(course=selected_course_obj).first() 
            ...
            if completion_status:
                 ...
                 quiz_passed = completion_status.quiz_passed 
                 quiz_taken = completion_status.quiz_taken   
...
def mark_lesson_complete():
    try:
        ...
        try:
            ...
            all_lessons_count = Lesson.objects(unit__in=units).count() 
            completion_status.reload() 
...
def generate_certificate():
    ...
    try:
        ...
        c.setFont("Helvetica-Bold", 24)
        c.drawCentredString(width / 2, height - 6 * inch, course.course_name) 
...
def mentor():
    if request.method == "POST":
         email = request.form['email']
         password = request.form['password']
         mentor_obj = Mentor.objects(email=email).first()
         if mentor_obj and bcrypt.check_password_hash(mentor_obj.password, password):
             if mentor_obj.status:
                 ...
                 session['profile_picture'] = mentor_obj.image_file 
                 flash("Login Successful", "success")
...
@app.route("/mentor/course/<course_id>/delete", methods=["POST"])
@role_required('mentor')
...
def add_unit(course_id):
    ...
    order = request.form.get('unit_order', 0) 
...
def add_lesson(unit_id):
    ...
    external_url = request.form.get('lesson_external_url') 
...
def content_upload():
    ...
    contents = Content.objects(course_name=course) 
    if request.method == "POST":
...
def mentor_dashboard():
    ...
    return render_template("Mentor_Dashboard_Simple.html",
...
def view_feedback():
    ...
    for feedback_item in mentor_feedbacks:
         category = feedback_item.course_category
...
def download_zip():
    ...
    files_to_download = request.args.getlist('files') 
